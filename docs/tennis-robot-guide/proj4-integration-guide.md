# Proj4 工作整理与接手指南：RK3588 与 SG2002 网球机器人

本文档面向接手网球机器人方向（Proj4"面向边缘智能的 AIOS"）的后来开发者与项目维护者，承担双重职能：**第 2 章是 Proj4 工作整理台账**——登记全部成果的位置（dev / next / 本地 / 外部）、验证状态与合入队列，是"整理 Proj4 工作"的目标交付物；**第 3 章起是接手路径**——从拿到小车硬件、构建并刷入内核、运行验证应用，到按工程纪律回馈主线。正文以 upstream `dev` 分支为读者基线：按本文走通主路径只需要 clone `dev` 就能获得的东西；不在 `dev` 上的材料集中登记在第 2 章台账里。
...
`dev` 上已经具备走通主路径的全部内核侧资产（板级配置、构建体系、采集与推理应用、板级测试用例）。不在 `dev` 上的材料分三类：已验证待合入的 `next` 内容（aka-rk3588 demo 应用、THP/调度/快路径一批）、只在本地工作区的未提交材料（全流程 tennis 应用、本文档所在目录）、外部依赖（SG2002 小核固件、Linux 底包镜像、各队决赛报告原件）。三者清单、现状与去向建议统一登记在第 2 章台账，不在正文分散说明。
...
| 材料 | 现状 | 建议去向 |
| --- | --- | --- |
| 本文档与各队评审文档 | 工作区未提交（本目录） | 整理后 PR 进 dev（文档批可先行） |
...
2. **文档批入库**：本文档与评审文档、SG2002 实操手册重建，PR 进 dev。
...
| 快速入门（官方） | `docs/docs/quickstart/starryos.md`（即文档站页面源码） | xtask 命令与板测通道细节 |
| 本文档与台账 | `docs/tennis-robot-guide/`（本目录） | Proj4 整理台账与接手指南 |
...
本文档是活文档：复现中发现的偏差（尤其【待验证】项）与台账的每次合入/拍板，直接修订对应小节并更新事实基线中的核对日期，让下一位接手者拿到的是最新状态。
