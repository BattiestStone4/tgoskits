...
Posad 还有 5 个改动停在待合并状态，方向集中在：缺页处理与页表维护（#1993、#2000、#2009）、大页拆分（#1990）、设备驱动的锁（#2003）。这个数按 2026-09-17 核对。

这批内容和已经合入的部分是同一档次，只是还没走完评审。对主线来说这是最省事的一块增量——作者还在，起点就是主线，不需要再找谁要代码，也不需要做分支比对。

另有四组已经由上游接续。性能测量那四个（#1577、#1601、#1602、#1603）由 #2274 整合，2026-09-13 合入 `dev`。从设备树读每核算力的 #2001、进程退出时回收设备锁的 #2072，按现在的分层重做成了 #2386 和 #2387，这两个也在 2026-09-13 当天合入 `dev`。线程分发与大小核摆放的 #1656 在 2026-09-17 关闭，由 #2434 承接：它从 #1656 的 head 整理出普通信号错误释放 ptrace stop 那处缺陷，rebase 到最新 `dev` 后按 Linux 的 TASK_TRACED 语义重构，同日合入 `dev`；#1656 原来的调度补丁没有跟着进来，因为 `axtask/run_queue.rs` 已被任务系统重构替代，直接合入会恢复过时机制。依赖 #2001 那份算力数据的 #2064 没有跟进，已经关闭且没有合入，要做得按 #2386 的新接口重写。同一批关闭的还有大小核性能测量工具的 #1658，也在 2026-09-13。
...
表里的数是按提交信息归类数出来的，一条提交同时涉及多个主题时只算一次，所以各行是粗略值；总数 108 是这条 `next` 分支相对 `dev` 的提交数，复核要分两段：从 `BattiestStone4/tgoskits` 取下来的 `next`（`9fe04e74e`）跑 `git rev-list --count <dev>..next` 得到 66，剩下 42 个还没推上去、就是前面说的那批，两段加起来才是 108。这三个数按 2026-09-17 核对。

这批里有一组编号（#2008、#2009、#1656、#1601、#1602）在 `dev`、`next` 和各功能分支的提交记录里都查不到，页表和大页的工作实际是以没有编号的提交落地的。#1577 和 #1603 是例外：整合性能测量那批的合并提交在正文里点到了它们，说吸收了这两份工作里的 ABI 部分、没有采用它们当时的分配器和 PID 模型。

这些编号对应的是上游的 PR，按 2026-09-17 核对的结论是：#2009 还开着，#1656、#2008、#1577 以及 #1601 到 #1603 已经关闭（#1656 的去向见 3.2），其中性能测量那四个由 #2274 整合进 `dev`。
...
第五步那 5 个改动虽然是待评审状态，但数量不少，排在这里可以让前面几批先把集成的流程跑顺。

最后一步容易被省掉，但它是这次比赛最不该丢的东西：验证方法比某个具体改动活得久，沉淀下来之后，下一批队伍的成果可以直接按同一套标准验收。

除了其他队伍的候选，仓库自己这边还有两件事要先做：把本地领先的 42 个提交推到远端并重新验证，以及补齐第 5.4 节里卡住的那两项（镜像打包模板和组装脚本）。
...
大部分内核工作不需要板子就能推进，按成本从低到高有三层：主机侧单元测试、内核测试、QEMU 系统测试。写回归测例时优先选这三层里最低的一层——能在主机上确定性复现的毛病不必上 QEMU，能上 QEMU 的不必等板卡。
