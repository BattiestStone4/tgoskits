...
Posad 还有 18 个改动停在待合并状态，方向集中在：线程分发与大小核摆放、页表维护顺序、透明大页、每线程 CPU 时间的并发安全、性能测量工具。
...
这批里有一组编号（#2008、#2009、#1656、#1577、#1601 到 #1603）在本地所有分支的提交记录里都查不到，页表和大页的工作实际是以没有编号的提交落地的。这些编号可能来自还在评审或没合并的 PR，排合入计划之前先到 GitHub 上确认它们的真实状态。
...
第五步那 18 个改动虽然是待评审状态，但数量不少，排在这里可以让前面几批先把集成的流程跑顺。
