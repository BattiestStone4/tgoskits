...
| 帧到指令 | 从采集到发出电机指令的延迟 | 各队取的百分位不同，有的给 p50，有的给平均值 |
...
| 其他 | 八线程跑分为 Linux 的 0.96 倍 | 输入链 16.9→8.9 ms | drive 成功率 94–98% | 三核墙钟约等于单核 | SD 卡写 185–213→13–24 ms |
...
Posad 还有 8 个改动停在待合并状态，方向集中在：线程分发与大小核摆放、缺页处理与页表维护、大页拆分、设备驱动的锁、大小核性能测量工具。

这批内容和已经合入的部分是同一档次，只是还没走完评审。对主线来说这是最省事的一块增量——作者还在，起点就是主线，不需要再找谁要代码，也不需要做分支比对。

另有三组已经由上游接续。性能测量那四个（#1577、#1601、#1602、#1603）由 #2274 整合，2026-09-13 合入 `dev`。从设备树读每核算力的 #2001、进程退出时回收设备锁的 #2072，按现在的分层重做成了 #2386 和 #2387，都还在评审。依赖 #2001 那份算力数据的 #2064，要跟着改到新的查询接口上。
...
**按优先级分层的轮转调度加优先级感知唤醒（火锅宇泡面）。** 每一步改动单独测一遍的结果是：只把单队列改成分层队列，推理的 p50 就从 1741 ms 降到 626 ms；再叠加唤醒时的抢占判断，整环总耗时从 1959 ms 降到 363 ms。这两个数字说明调度策略在这道题上的分量比想象中大得多。要注意的是，他们加的是一套新机制，不是第三套调度器，主线已有的两种调度器不能重复。
...
**四级验证阶梯（大象）。** 把测试分成四层：真实模型加字节级协议断言、静态链接自检、多核基准、真机跑。好处是没有硬件也能回归。

**整环基准体系（火锅宇泡面）。** 用两个模式（一个只观察、一个真驱动）加九项阶段计时，输出 CSV，再配合每次只改一处的版本对照，能干净地判断每一步优化各自的贡献。
...
主线是 `dev`，所有改动都通过 PR 合进这里，克隆下来就能用，本篇说的"已经在主线"指的就是它。和它并行的是 `next`：一个集成用的缓冲区，把各个功能分支上验证过的工作合到一起做集成测试，它可以随时推倒重建，所以不要在上面写只属于它的代码。开发本身在各自的 `feat/*` 分支上做。
...
一块是**板级配置**。两块平台的板级描述文件、设备树、镜像打包模板都在 `configs/board/` 下，已经入库，加新板子能力优先加配置项而不是改代码路径。
...
`next` 上的内容按主题分成六组，加起来一百多个提交。合并前有两件事要注意：本地这份 `next` 领先远端 42 个提交还没推上去，得先推并重新验证；另外要按主题拆成多个 PR 分批合，每批合完直接更新这张表。

| 主题 | 内容 | 量级 |
| --- | --- | --- |
| 应用与基准 | 一个预编译的机器人演示应用、大小核基准与板级测试架子 | 2 项 |
| 驱动 | USB 主控生命周期、SD 卡控制器复位、驱动框架三项、图像解码失败保护、虚拟化存储、串口运行时、USB 从机拆分 | 约 25 个提交 |
| 调度与跑核 | 按核容量分配线程、每核容量表、每线程 CPU 时间支持多核、调度内省 | 约 27 个提交 |
| 内存与大页 | 匿名大块透明大页、大页拆分、大页切分顺序、中间页表回收前刷新、驻留计数、重映射接口、页框分配 | 约 28 个提交 |
| 系统调用快路径 | 快速互斥表分片、写文件去冗余、安全检查快路径、无定时器进程跳过轮询 | 约 17 个提交 |
| 测试 | 时间管理器测试、透明大页测试、基准架子修复 | 约 28 个提交 |

这批里有一组编号（#2008、#2009、#1656、#1577、#1601 到 #1603）在本地所有分支的提交记录里都查不到，页表和大页的工作实际是以没有编号的提交落地的。这些编号对应的是上游的 PR，2026-09-13 核对的结论是：#2009 和 #1656 还开着，#2008、#1577 以及 #1601 到 #1603 已经关闭，其中性能测量那四个由 #2274 整合进 `dev`。
...
| C10 | 大象 | 四级验证阶梯 | 提成独立工具；模型和库走下载脚本，不进仓库 |
| C11 | 火锅宇泡面 | 整环基准体系 | 作为板级应用的测试样板，和它的应用一起评估 |
...
| 大象 | 终端对空格和 Tab 的处理、回显时换行转换 | 已放行，对应 #1484、#1922 |
| 大象 | 终端控制命令 | 已有且更全 |
...
第五步那 8 个改动虽然是待评审状态，但数量不少，排在这里可以让前面几批先把集成的流程跑顺。
...
写代码前要读的三份规范、以及具体的命令，见 [上手指南](./getting-started.md) 第 7 章。
...
| 编码规范 | `docs/guideline/` 下三份 | 写代码前按情况选读 |
