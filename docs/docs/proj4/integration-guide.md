---
sidebar_position: 1
sidebar_label: "工作整理与接手指南"
title: "Proj4 工作整理与接手指南"
---

# Proj4 工作整理与接手指南

本指南面向接手网球机器人方向（Proj4"面向边缘智能的 AIOS"，覆盖 RK3588 与 SG2002 两个平台）的后来开发者与项目维护者，承担双重职能：**第 2 章是 Proj4 工作整理台账**——登记全部成果的位置（dev / next / 本地 / 外部）、验证状态与合入队列，是"整理 Proj4 工作"的目标交付物；**第 3 章起是接手路径**——从拿到小车硬件、构建并刷入内核、运行验证应用，到按工程纪律回馈主线。正文以 upstream `dev` 分支为读者基线：按本文走通主路径只需要 clone `dev` 就能获得的东西；不在 `dev` 上的材料集中登记在第 2 章台账里。

> 事实基线：upstream `rcore-os/tgoskits` dev 主线。本文核对基于本地 dev 镜像 `3049b0c7be`（2026-08-25，落后 upstream 约 59 个提交）；next 集成树核对至 `02d7b870f0`（2026-09-03）。标有【待验证】的步骤尚未经真机完整复现，执行时以实际输出为准，并把偏差反馈给维护者修订本文。

## 1. 总览

### 1.1 项目定位与分支模型

tgoskits 是 ArceOS、StarryOS、Axvisor 的统一仓库，内核、驱动、组件、内存与虚拟化代码按层分目录复用。网球机器人方向在 StarryOS 上打通"摄像头采集 → NPU 推理 → 状态机决策 → 电机控制"闭环，并在过程中持续优化内核。仓库的分支模型决定你从哪里开始、往哪里提交：

| 分支 | 含义 | 规则 |
| --- | --- | --- |
| `dev` | upstream 主线（本文读者基线） | 所有合入经 PR，克隆即用 |
| `next` | 集成缓冲区：`dev` + 各源头分支已验证工作 | 可推倒重建，不在其上写独有代码 |
| `feat/*` | 各功能源头分支 | 归开发者维护，`next` 从这里吸收 |

三层的日常动线是：从 `dev` 切出 `feat/*` 开发，验证后进 `next` 做集成测试，最终以 PR 合回 `dev`。接手后的第一个动作是读本文第 2 章台账了解全局，再按下图选择入口：

```mermaid
flowchart TD
  A[克隆 dev, 读第 2 章台账] --> B{手上有硬件吗}
  B -- 有 RK3588 小车 --> C[第 6-8 节: 构建, 写卡, 跑应用]
  B -- 有 SG2002 板 --> D[第 6-8 节 SG2002 部分]
  B -- 暂无硬件 --> E[第 9 节: QEMU 与测试套]
  C --> F[第 2 章领任务: 合入队列或候选]
  D --> F
  E --> F
  F --> G[第 10 节: 回归测例, 验证, 提 PR]
```

图中的三条入口最终汇合到同一条贡献路径：先跑通存量（确认环境与板卡正常），再从台账领一个不与主线重复的任务，最后按工程纪律提交。

需要说明一个历史包袱：此前的集成台账 `INTEGRATION.md` 与集成流程文档 `docs/tgoskits-next/` 从未提交进任何分支，已随一次工作区清理丢失。本文第 2 章承接其职能（内容依据 git 取证与维护者记录重建）；今后台账随本文入库并随合入更新，避免再次出现"只存在于一个人机器上"的状态。

### 1.2 材料边界

`dev` 上已经具备走通主路径的全部内核侧资产（板级配置、构建体系、采集与推理应用、板级测试用例）。不在 `dev` 上的材料分三类：已验证待合入的 `next` 内容（aka-rk3588 demo 应用、THP/调度/快路径一批）、未提交进 `dev` 的本地材料（全流程 tennis 应用、本指南与评审文档）、外部依赖（SG2002 小核固件、Linux 底包镜像、各队决赛报告原件）。三者清单、现状与去向建议统一登记在第 2 章台账，不在正文分散说明。

## 2. Proj4 工作整理台账

本章是 Proj4 收尾的整理台账：登记每一项工作的当前位置、验证状态与建议去向，供维护者排合入计划、供接手者直接领任务。状态分三档——已在 `dev`（2.1，无需动作）、在 `next` 待合入（2.2）、未入库待拍板（2.3）；另有其他队伍候选一条接续线（2.4）与建议推进顺序（2.5）。每完成一次合入或拍板，直接更新对应表格并在提交说明里记录。
...
核对方法是双重的：PR 编号给出来龙去脉，代码位置用于在现树上确认（例如 `drivers/npu/rockchip-npu/src/gem.rs` 的 `GemPool`）。计划做某子系统优化时，先读对应模块现状再立项；2026-08 之后的调度/THP/快路径改进不在 dev，在 `next` 待合入（见 2.2）。

### 2.2 在 next 待合入 dev

`next`（`02d7b870f0`，2026-09-03）领先 dev 镜像 108 个提交，按主题归组如下。合并前注意两点：本地 `next` 领先 fork 上的 `origin/next` 42 个提交（含合并残迹修复 `02d7b870f0`），需先 push 并过 T0/T1 复验；分批合入时按主题拆 PR，每批合入后直接更新本表（打勾或移除）。
...
以下材料未提交到任何分支（纯工作区、已丢失或外部依赖），是 Proj4 整理的主要待办。每项给出建议去向，落地时按第 10 章流程走：

| 材料 | 现状 | 建议去向 |
| --- | --- | --- |
| 本指南与各队评审文档 | `docs/docs/proj4/`（文档站本分类，未进 dev） | 文档批 PR 先行合入 dev |
| 原集成台账 `INTEGRATION.md`、`docs/tgoskits-next/` | 从未入库，已丢失 | 职能由本文第 2 章承接；简版台账随本文入库 |
...
| `os/StarryOS/configs/board/orangepi-5-plus.its` | dev/next 均缺（uimg 生成缺口，见 6.2） | 从构建产物恢复并入库，独立小 PR |
...
| 底包镜像、固件二进制 | 本地/外部 | 不入库，在文档中登记获取方式（见 7.1） |
| 各队决赛报告原件 | `~/Downloads/附件下载_Proj4 每周同步共享文档/`（含 zip 备份） | 永不提交进 git；整理工作以它为依据 |

表中 `.its` 与 image.fit 两项是 blocking 级小缺口——不补则 RK3588 的 uimg 生成不可复现；文档两项是本批 PR 的主体。

### 2.4 其他队伍候选

决赛五队的评审结论（YatSenOS、火锅宇泡面、大象飞出冰箱外、重返未来、Posad 对照）筛出 15 项合并候选（C01-C15），其中 P0 六项建议优先评估。详细判定、证据与页码见同分类的 [各队决赛评审](./peer-teams-review.md)：
...
综合验证状态与依赖关系，建议的推进顺序如下；每步的验证要求见第 10 章，对外动作（push、PR）先与维护者确认：

1. **push `next` 并过 T0/T1**：42 个本地提交未推，合入队列全部以此为前提。
2. **文档批入库**：本指南与评审文档、SG2002 实操手册重建，PR 进 dev。
3. **小缺口修复**：`orangepi-5-plus.its` 恢复入库、image.fit 组装脚本化（解除 RK3588 uimg 不可复现问题）。
4. **next→dev 分批合入**：按 2.2 六个主题拆 PR（驱动/内存/调度/快路径/测试/应用），每批带回归证据。
5. **存疑编号复核**：2.2 末列的 PR 号到 GitHub 复核，确认在审/未合并状态后重排计划。
6. **候选接续**：按 2.4 流程从 C05 开始落地。

## 3. 快速上手卡

以下三组命令分别对应两种硬件平台与无硬件场景，是接手路径的浓缩入口；每组命令只依赖 `dev` 上存在的资产。在你的开发机上进入仓库根目录执行。
...
SG2002（荔枝派 Nano / AKA-00 车板）最小路径——编译后手工换镜像、写卡、U-Boot 手动引导（详见 7.2）：
...
三组命令的成功判据不同：两个硬件平台以串口出现 StarryOS shell 且应用验证标志出现为准（见第 8 节各表）；QEMU 路径以命令正常退出、测试套输出通过为准。

## 4. 硬件与启动链路

### 4.1 RK3588 小车构成

主控是 OrangePi 5 Plus（RK3588：8 核 4×A76+4×A55，NPU 6TOPS），整机由主控板、UVC 摄像头与三路执行器组成。执行器配置来自全流程 tennis 应用（目前为本地未提交材料，见 2.3）：底盘电机经 DRV8833 驱动、由 RK3588 的 PWM sysfs 四通道控制；另有 ESP32-C3 经 UART 挂接的底盘方案与 ZP10S 串行总线舵机机械臂，后端可在应用内按虚拟/真实切换。SD 卡上 Ubuntu rootfs 与 StarryOS 共享，应用资产预置在 rootfs 分区。
...
| 底盘电机 DRV8833 | PWM sysfs 四通道 | tennis 应用执行器后端（2.3） |
| ESP32-C3 底盘 / ZP10S 舵机臂 | UART 串行总线 | tennis 应用执行器后端（2.3） |
...
QEMU 窗口/串口出现 StarryOS shell 且进程正常退出或可交互，即环境可用。若这一步失败，问题在工具链或 QEMU 版本，与板卡无关；对照仓库 `README.md` 与在线文档站（见第 12 章）的环境章节排查后再继续。
...
标准构建命令与产物校验如下；该命令在 2026-09-03 集成轮验证通过：
...
uimg 由构建系统发现与 toml 同名的 `.its` 模板后调用 `mkimage` 生成（`scripts/axbuild/src/starry/build.rs`）。注意一个已知缺口：`configs/board/` 下没有 `orangepi-5-plus.its`（dev 与 next 均无，三个 `.its` 都属于 SG2002），uimg 生成依赖持久化 defconfig 中的副本——若构建未产出 uimg，说明 `.its` 缺失，此时可直接使用 `starryos.bin` 组装 FIT。该缺口已列入 2.3 合入队列（恢复 `.its` 入库）。
...
`mkimage -l` 的输出里 Load/Entry 必须都是 `0x80200000`；若为 0 说明 `.its` 解析失败，产物无法引导（对应 11.2 的 EPC=0 故障）。
...
具体部署路径与验证标志见第 8 节两张应用表。

## 7. 手工写卡与上板

### 7.1 通用原则

两个平台的"换内核"都是替换启动介质上的镜像文件，不动固件：SG2002 的 fip（含 SDIO PHY 等底层初始化）与 RK3588 的 U-Boot 都保持底包原样，只换 StarryOS 内核镜像。写卡操作的三条铁律：任何写入后 `sync` 并安全弹出再断电（跳过 sync 硬断电会损坏 ext4 rootfs，修复流程见 11.2）；优先写 FAT 分区而非 ext4（降低损坏面）；保留一份可启动的底包镜像，坏了直接重刷整卡而不是在坏卡上叠写。
...
底包镜像属于外部资产不入库（见 2.3），获取方式：SG2002 向维护者索取现成镜像或从 vendor 底包自制；RK3588 用 OrangePi 官方 Ubuntu 镜像。

### 7.2 SG2002 流程

流程要点如下（历史上有一份更完整的实操手册，已丢失待重建，见 2.3）。先在 Linux 或 `--privileged` 容器里挂载镜像换内核，再整卡写入 SD：
...
# 3. 挂载 rootfs 分区, 预部署应用资产 (见 8.1 表), sync 后弹出
```
...
四个应用构成从采集到全流程的阶梯，注意它们的分支位置不同：前两个在 `dev` 上，aka demo 在 `next`，全流程 tennis 应用还在本地工作区（未提交，见 2.3）。验证标志是判断"跑通了"的客观判据（板级 CI 也用这些正则）：
...
`aka00-tennis-yolo` 是 `dev` 上 SG2002 的"推理链路自检"：不接摄像头，用预置图片验证 CV181x TPU 推理正确性，输出 `AKARS_TENNIS_VALIDATE_PASS` 即通过。WiFi 链路用 `-wifi` 变体 toml 构建，AIC8800 固件在构建时按 pinned commit 自动下载校验，无需手工放置。cvi 邮箱三件套工具受小核固件外部依赖阻塞，暂留分支（见 2.3）。
...
| 层 | 内容 | 执行方式 |
| --- | --- | --- |
| T0 | 受影响配置的 Docker 全量编译 | 集成脚本（Docker 容器内全量 build） |
| T1 | QEMU + 单元 + std + starry 测试套 | CI 与本地 `cargo xtask` 可复现 |
| T2 | 实板功能验证 | 手工/板卡服务 |

`next` 的吸收流程（原 `docs/tgoskits-next/README.md` 已丢失，要点在此重建）：dev 快进 upstream → 从 dev 重建 `next` → 逐个 merge 来源分支并记 SHA → T0/T1 → 打 tag `tgoskits-next-YYYYMMDD` → 更新台账。源头分支与集成树的分工是：fix 与 feat 都在 `feat/*` 上做，`next` 只做集成验证，冲突回源头分支解决。
...
构建期问题多数指向配置机制（见 6.1）。下表汇总最高频的几类：
...
构建失败先确认自己在容器或齐备的本机环境里（第 5 节），再看是否踩了上表配置类问题，最后才怀疑代码。
...
| 重启后回到旧内核 | autoboot 走 p1 `boot.sd` | 每次手动引导，见 7.2 |
| 串口乱码 | 波特率错或 fip 与板型不匹配 | 核对 115200/1500000 与 fip 身份 |
| rootfs mount 失败/文件消失 | 写后未 sync 硬断电致 ext4 损坏 | 重刷整卡；预防见 7.1 |
...
| WiFi 大包失败/CRC 错 | fip 与板型不匹配 | 换回对应板的 fip（见 7.4） |
| NPU demo 失败 | rootfs 资产缺失或未 sync | 对照 8.1 表补齐预置 |
| 传文件没有 scp | 板上无 sftp-server | 用 8.3 的 cat 管道 |

仍无法定位时，串口全程日志 + 复现步骤是求助的最小信息集；内核日志被 shell 掐断的场景参考维护者已知的 console 输出问题排查记录。

## 12. 资源索引

以下材料构成这个方向的完整知识地图，按用途组织。优先读前两行（环境与快速入门），做内核优化前读评审材料与本文台账，写代码前读规范：

| 材料 | 位置 | 用途 |
| --- | --- | --- |
| 仓库 README | `README.md`、`README_CN.md` | 环境与三条系统路径入口 |
| 官方文档站 | https://rcore-os.cn/tgoskits/ | 在线阅读版（quickstart、架构等） |
| 文档站源码 | https://github.com/rcore-os/tgoskits/tree/dev/docs | 文档贡献入口（Docusaurus） |
| 快速入门（官方） | 站内 [StarryOS 快速上手](../quickstart/starryos.md) 页（源码 `docs/docs/quickstart/starryos.md`） | xtask 命令与板测通道细节 |
| 本指南（本页） | 文档站 Proj4 分类（`docs/docs/proj4/`） | Proj4 整理台账与接手指南 |
| Proj4 各队决赛评审 | 站内 [各队决赛评审](./peer-teams-review.md) 页（同分类） | 候选合并队列与现状判定 |
...
| Posad 决赛报告 | `~/Downloads/附件下载_Proj4 每周同步共享文档/`（本地） | 优化方法论与上游化过程 |

本指南是活文档：复现中发现的偏差（尤其【待验证】项）与台账的每次合入/拍板，直接修订对应小节并更新事实基线中的核对日期，让下一位接手者拿到的是最新状态。
